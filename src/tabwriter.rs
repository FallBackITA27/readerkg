...
fn default() -> Self {
    Self{str: String::new(), col_lengths: [2;255], current_col:0, tmp_current_col_length: 0}
}
}
...
impl Write for TabsWriter {
    fn write_str(&mut self, s: &str) -> std::fmt::Result {
        ...
        for char in s.chars() {
            match char {
                '\t' => {
                    self.col_lengths[self.current_col] = std::cmp::max(self.tmp_current_col_length, self.col_lengths[self.current_col]);
                    self.current_col+=1;
